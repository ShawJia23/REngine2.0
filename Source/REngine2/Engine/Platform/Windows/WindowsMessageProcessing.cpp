...
LRESULT CALLBACK EngineWindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONUP:
		...
		return 0;
	case WM_RBUTTONDOWN:
		RMouseDownDelegate.Broadcast(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		return 0;
	case WM_RBUTTONUP:
		RMouseUpDelegate.Broadcast(GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		return 0;
	case WM_MOUSEMOVE:
	...
	case WM_SIZE:
	{
		int viewPortW = LOWORD(lParam);
		int viewPortH = HIWORD(lParam);
		ResizeScreenDelegate.Broadcast(viewPortW, viewPortH);
		return 0;
	}
